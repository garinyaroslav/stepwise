...
public class CreateGroupDto {
    ...
    @Pattern(regexp = "[А-Яа-яA-Za-z]{1,4}-\\d{3}", message = "name of group must be in format: 4 letters, hyphen, 3 digits (e.g., ПИН-122 or ПМИз-123)")
    private String name;

    @NotNull(message = "Students ids is required")
    @NoDublicatesInCollection(message = "Student IDs must be unique")
    private List<@NotNull(message = "Student ID cannot be null") @Positive(message = "Student ID must be a positive number") Long> studentIds = new ArrayList<>();
